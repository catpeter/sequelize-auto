...
function AutoSequelize(database, username, password, require, options) {
...
AutoSequelize.prototype.build = function(callback) {
  ...
  function mapTable(table, _callback){
    // pyw 为了获得comment
    let sql = "SELECT COLUMN_NAME AS fieldName, column_comment AS comment FROM INFORMATION_SCHEMA.Columns WHERE table_name = '" + table  + "';";
        ...
        if (result && result.length) {
          result.forEach(element => {
            if (element && element.comment) {
              fields[element.fieldName].comment = element.comment
            }
          });
        }
...
AutoSequelize.prototype.run = function(callback) {
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      // 按需处理的tableName pyw
      var tableName = self.options.camelCase ?  _.camelCase(table): replaceUnderlineAndfirstToUpper(table);
      ...
      if(!self.options.typescript){
        ...
        text[table] = "var sequelize = require('../config/" + self.require + "');\n";
        text[table] += "var Sequelize = require('sequelize');\n\n";
        text[table] += "var " + tableName +  " = sequelize.define('" + tableName + "', {\n";
      ...
      _.each(fields, function(field, i){
        ...
        var fieldName = self.options.camelCase ? _.camelCase(field) : replaceUnderlineAndfirstToUpper(field);
        // change pyw
        if (self.tables[table][field].comment) {
          text[table] += spaces + spaces + "// " + self.tables[table][field].comment  + "\n";
        }
...
AutoSequelize.prototype.write = function(attributes, typescriptFiles, callback) {
  ...
  function createFile(table, _callback) {
    var fileName = self.options.camelCaseForFileName ? _.camelCase(table) : replaceUnderlineAndfirstToUpper(table) ;
...
function  replaceUnderlineAndfirstToUpper(srcStr, org = '_', ob = '') {
    var newString = "";
		var first = 0;
		if (srcStr.indexOf(org) !== -1) {
			first = srcStr.indexOf(org);
			if (first !== srcStr.length ) {
				newString = newString + srcStr.substring(0, 1).toLowerCase() + srcStr.substring(1, first) + ob;
				srcStr = srcStr.substring(first + org.length, srcStr.length);
				srcStr = srcStr.substring(0, 1).toUpperCase() + srcStr.substring(1);
      }
		}
    newString = newString + srcStr;
    if (first !== 0 && first !== srcStr.length) {
      newString = replaceUnderlineAndfirstToUpper(newString);
    } else {
      newString = newString.substring(0, 1).toLowerCase() + newString.substring(1, newString.length)
    }
		return newString;
